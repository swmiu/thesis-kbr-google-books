...
function loadPage(pageName) {
    ...
    const pages = {
        ...
        'methodology': `
            ...
            <p>To transform the 1,000 MARCXML bibliographic records into an interactive dashboard website, a data pipeline was created using Python Jupyter Notebooks and OpenRefine. The pipeline comprises six stages, from sampling to web deployment.</p>

            <h2>STEP 0: DATA ACQUISITION AND SAMPLING</h2>
            <p>On 23 April 2026, the internship mentor exported the source file from KBR's internal Google Books Project management application. The exported data was drawn specifically from the "Digitise by Google Books" category, which contains bibliographic entries whose metadata has been reviewed and updated manually by Google Books Project team members, ensuring a higher data quality.</p>
            <p>Sampling was restricted to the two main collection sections: M-SLZ (<em>Magasin Salle de Lecture Générale</em>, General Reading Room stacks) with 70,230 records (81.1%), and M-RP (<em>Magasin Réserve Précieuse</em>, Precious Works stacks) with 16,337 records (18.9%), totalling 86,567 records. The remaining 69 records came from minor sections and were excluded due to their very small share (&lt; 0.1%) and heterogeneous metadata formats.</p>
            <p>The data was then deduplicated at IDN level, as the source listed records at the level of individual volumes or copies, leaving 68,036 unique records. A proportional stratified random sample of 1,000 records was drawn, allocating 810 to M-SLZ and 190 to M-RP, with a fixed random seed to ensure full reproducibility. This step resulted in a list of 1,000 unique IDNs. The internship mentor then coordinated with colleagues to export the corresponding MARCXML records from KBR's catalogue system (Syracuse) based on this list. The final exported MARCXML file serves as the input for Step 1.</p>

            <h2>STEP 1: EXTRACTION</h2>
            <p>The 1,000 MARCXML records were parsed using Python's ElementTree library, going through each &lt;record&gt; element in the XML file to extract structured fields into seven relational CSV files. The extraction logic was designed to handle both one-to-one attributes and multi-valued relationships, following the Third Normal Form (3NF) principle.</p>
            <p>For most fields, extraction was straightforward: the book identifier from 001, the title from 245 $a, cities from 264 $a, and country codes from 044 $a. Three fields required extra logic, namely the publication year, publisher, and language.</p>
            <p><strong>Publication year</strong> was drawn from 008 positions 07&ndash;10 (Date 1). While Date 1 marks the beginning year and positions 11&ndash;14 (Date 2) mark the end year for works published over a span of time, Date 1 was prioritised in this research to focus on the first appearance of the work. Where this field yields no valid date, 264 $c serves as a fallback, recovering 4 of the 17 initially null records. The remaining 13 records were excluded from temporal analysis but kept for the linguistic and geographic dimensions, yielding a final total of 987 datable records spanning 1602 to 1901.</p>
            <p><strong>Publisher names</strong> were extracted from authority-controlled fields (100, 110, 700, 710, 720), taking subfield $a only where $4="pbl", rather than from field 264 $b. The field 264 $b is a transcription field which records names exactly as they appear on the title page, without any assignment of roles such as publisher and printer, whereas the authority-controlled fields explicitly identify the entity's role and link to a standardised entity. This yielded publisher information for 452 of the 1,000 records, a coverage rate of 45.2%, prioritising consistent data quality over breadth.</p>
            <p><strong>Language classification</strong> was based on codes extracted from 041 $a. Records containing multiple language values are consolidated under a single "multiple languages" category, which aligns with the ISO 639-2 standard code mul.</p>

            <h2>STEP 2: CLEANING</h2>
            <p>After creating the required CSVs, the three entity tables underwent different approaches depending on the characteristics of the metadata and the ultimate visualisation goals.</p>
            ...
            <p>This step also created a temporal periodisation scheme for the publisher treemap, reflecting the concentration of publisher records observed during 1861&ndash;1890 rather than imposing uniform time intervals.</p>

            <h2>STEP 4: VISUALISATION</h2>
            <p>Six interactive visualisations were generated using Plotly and Folium, exported as standalone responsive HTML files with interactive tooltips. The choice of chart type for each dimension was driven by the respective analytical question and the structure of the data, as well as the visual emphasis the author wanted to convey to the readers. All charts follow a colour palette that echoes KBR's official website, with visual consistency and readability as the two main considerations.</p>
            ...
            <p>The <strong>publisher treemap</strong> presents the relative place of each publisher in the market, from period overview to individual publisher to individual book title. To avoid hundreds of nearly invisible tiles, a threshold of at least two publications within a given period was implemented, retaining the top 30 period-publisher segments by volume.</p>
            <p>The <strong>publisher language profile</strong> uses a stacked horizontal bar chart to observe whether individual publishers tend to specialise in a single language or were more multilingual. This chart reuses the identical filtering logic and publisher subset as the treemap, ensuring both publisher visualisations were based on the same subset of the sample. The language colours match those used in the language pie chart to maintain a coherent visual experience.</p>
            <p>The <strong>cities map</strong> is an interactive Folium map built from the coordinates harvested through Wikidata reconciliation. Each city is represented as a circle marker whose radius is scaled by the square root of its publication count, so that the perceived area of each marker remains proportional to the volume, with a minimum of 4 pixels to ensure that even single-publication cities remain visible and clickable.</p>

            <h2>STEP 5: WEBSITE AND DEPLOYMENT</h2>
            <p>The six HTML chart files were integrated into a static website built with HTML, CSS, and JavaScript, deployed on Vercel via GitHub. Each chart is embedded as an iframe within the page layout for easier integration. The website uses a sidebar navigation structure with six sections corresponding to the analytical dimensions.</p>
            <p>The visual design of the website centred on one key consideration: since the six charts are already rich in colour and diverse in shapes, the website body adopts a neutral, low-key colour scheme with warm tones for the content background and sidebar, paired with a clean and minimalist layout that avoids competing with the charts for visual attention. The same minimalist aesthetic was also applied to typography. Jost was used for headings, with Source Serif 4 for body text, both loaded via Google Fonts.</p>
            <p>The page banners are custom illustrations by Yung-Heng Wong, added to introduce the theme of the project. They were integrated using the HTML &lt;picture&gt; element to serve optimised images for desktop and mobile users.</p>
            <p>The layout adapts across desktop, tablet, and mobile breakpoints; on narrow viewports, chart containers enable horizontal scrolling to preserve chart readability without distorting the visualisations.</p>
            <p>During development, text and background colour combinations were validated using the WebAIM Contrast Checker against the WCAG 2.1 Level AA contrast minimum. Finally, following a basic accessibility audit, the heading hierarchy was corrected to ensure proper nesting.</p>
