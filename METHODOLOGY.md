...
* **Extraction & Modeling Heuristics**:  
  ...
  * **Language**: Extracted from the first occurrence of subfield `041 $a`. If a record contains more than one language value within `041 $a`, it is systematically categorized under a unified string as `"multiple languages"`. This decision aligns with the ISO 639-2 standard code `mul` (Multiple languages), defined by the Library of Congress for materials containing multiple languages. It also streamlines linguistic aggregation during data visualization, thereby optimizing collection-level statistical representation.  
...
* **Relational Schema (3NF Target Architecture \- 7 CSV Files)**:  
  ...
  * `book_publishers.csv` (`book_id`, `publisher_name`, `source_field`) → *Bridge table for multi-publisher networks.*  
...
* **2b. OpenRefine Semantic Enrichment (Targeted Entity Reconciliation)**  
  ...
  * **Logic**:  
    * **Geospatial Reconciliation & Enrichment**:
    ...
    For entities lacking valid Wikidata matches or geospatial attributes, latitude and longitude values were intentionally preserved as `null` to maintain data integrity and prevent programmatic speculation on uncertain geospatial locations. Geospatial coordinates were retrieved through OpenRefine's "Add columns from reconciled values" feature, which queries Wikidata properties for each matched entity.
...
* **Analytical Dimensions & Plot Types, and Algorithmic Logic:**  
  1. **Temporal Analysis**: Plotly Line Chart illustrating the temporal trajectory of publication activity based on the primary year captured from `008 pos. 07-10`. An embedded range slider allows readers to zoom into specific periods across the 300-year span. Year data is drawn directly from books.csv and contains one row per book by design.  
  2. **Linguistic Analysis**: Language Distribution — Plotly Pie chart displaying collection holdings, explicitly isolating multi-lingual records using the `"multiple languages"` heuristic derived from field `041 $a`. Language data is likewise drawn directly from `books.csv`, with each book carrying exactly one language value. 
  ...
  6. **Micro-Geospatial Mapping**: Cities Map — Interactive Folium map leveraging coordinates harvested from Wikidata, utilizing the exact `latitude` and `longitude` pairs harvested via OpenRefine. Each city is represented as a `CircleMarker` where the radius is scaled using a square root transformation of publication volume, with a minimum of 4 pixels for visibility and a maximum of 30 pixels to prevent overlap. This scaling preserves the relative ordering of cities while compressing the range to ensure smaller publication centres remain visible and interactive. 
