...
* **Extraction & Modeling Heuristics**:  
  ...
  * **Countries (Multi-valued Relational Extraction)**: Extracted from MARC field `044 $a` (Country of publication code). Similar to cities, a bibliographic item can represent co-publications spanning multiple geopolitical entities, meaning field `044 $a` may contain multiple values. To prevent data redundancy and support accurate aggregation, a many-to-many bridge table is implemented.  
...
* **2a. Python Baseline Preprocessing (Syntactic Cleaning)**  
  ...
  * **Logic**: Executing canonical character normalization, including trimming leading/trailing whitespaces, collapsing internal duplicate spaces, and converting name strings to Title Case for syntactic alignment.  
* **2b. OpenRefine Semantic Enrichment (Targeted Entity Reconciliation)**  
  * **Tools**: OpenRefine, Wikidata Reconciliation Service.  
  * **Targets**: Pre-processed entity arrays (`cities.csv`, `publishers.csv`). *Note: `countries.csv` is bypassed in this step to optimize processing efficiency, as MARC 044 codes are already standardized and will be mapped programmatically via Python.* In Step 3, the 044 codes in `countries.csv` will be mapped to human-readable country names (e.g., 'be' → 'Belgium') using a MARC 21 country code reference table built into the Python merge script.  
  * **Logic**:  
